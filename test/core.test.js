...
describe("sites", () => {
    ...
    async function testNext(page, fromSelector, nextEval, expectedValue, params) {
        ...
        const from = await page.$eval(fromSelector, getTop);
        const actualValue = await page.evaluate((from, params, fnText) => {
            const next = nextTarget(from, params);
            const fn = new Function(' return (' + fnText + ').apply(null, arguments)');
            return fn.call(null, next);
        },from, params, nextEval.toString());
        assert.equal(expectedValue, actualValue);
    }

    async function testNextTextContent(page, fromSelector, nextTextContent, params) {
        await testNext(page, fromSelector, e => e.textContent, nextTextContent, params);
    }

    async function testNextTop(page, fromSelector, nextSelector, params) {
        const expectedTop = await page.$eval(nextSelector, getTop);
        await testNext(page, fromSelector, getTop, expectedTop, params);
    }
